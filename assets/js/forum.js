    ...
    renderTopics: function(topics) {
        ...
        const html = topics.map(t => {
            ...
            const imgHtml = (t.gambar) ? `
                <div class="mb-3">
                    <div class="forum-image-preview-wrapper shadow-sm" onclick="openLightboxModal('${primaryUrl}', '${(t.judul || '').replace(/'/g, "\\'")}')">
                ...
                <div class="col topic-card-item" data-topic-id="${t.id}" data-visibility="${t.visibility || 'public'}">
                    <div class="forum-topic-card p-3 p-md-4 rounded-4 shadow-sm bg-white border h-100 d-flex flex-column justify-content-between">
                        <div>
                            ...
                            <p class="text-secondary mb-3 small" style="font-size:0.83rem; line-height: 1.48; color: #475569; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${t.konten_preview}...</p>
        ...
        const html = comments.map(c => {
            ...
            const cmtImg = (c.gambar) ? `
                <div class="mt-2 text-start">
                    <div class="forum-image-preview-wrapper shadow-sm d-inline-block" style="max-width: 320px;" onclick="openLightboxModal('${cPrimaryUrl}', 'Lampiran Balasan Komentar')">
                        <img src="${cPrimaryUrl}" onerror="this.onerror=null; this.src='${cFallbackUrl}';" alt="Lampiran Balasan" style="max-height: 180px; object-fit: cover;">
                        <div class="forum-image-overlay">
                            <div class="lens-icon-btn"><i class="bi bi-zoom-in"></i></div>
                        </div>
                    </div>
